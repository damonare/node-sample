...
exports.show = exports.upload = exports.start = undefined;
...
var start = exports.start = function start(response, postData) {
    console.log('Start!!!');
...
//引入外部模块，数据库
var upload = exports.upload = function upload(response, request) {
    ...
    form.parse(request, function (error, fields, files) {
        ...
        */
        ...
        var readStream = _fs2.default.createReadStream(files.upload.path);
        //返回一个新建的 WriteStream 对象
        var writeStream = _fs2.default.createWriteStream("./1.jpg");
        readStream.pipe(writeStream);
        readStream.on('end', function () {
            _fs2.default.unlinkSync(files.upload.path);
        });
        //如果您没有安装mongodb数据库或是使用的其他数据库记得将下面这句注释
        (0, _database.insert)('1.jpg', files.upload.path);
        // 发送 HTTP 头部
        // HTTP 状态值: 200 : OK
        // 内容类型: text/html
        response.writeHead(200, {
...
var show = exports.show = function show(response, postData) {
    console.log("Request handler 'show' was called.");
    //浏览器输入http://localhost:8888/show查看效果
    _fs2.default.readFile("./1.jpg", "binary", function (error, file) {
        if (error) {
